...
export default function ProductDetail() {
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/api/products/${id}`)
      ...
      .then(data => {
        setProduct(data)
        // Fetch related products (we just fetch all and filter for now)
      ...
      .then(all => {
        if (product) {
          ...
          // If we use the all products directly inside the chain
          setRelatedProducts(all.slice(0, 3))
        }
      })
      .catch(err => {
        console.error(err)
  ...
  if (loading) {
    return <div className="container" style={{ textAlign: 'center', padding: 'var(--space-3xl)' }}>Cargando...</div>
  }
  ...
  if (!product) {
    return (
      <div className="container" style={{ textAlign: 'center', padding: 'var(--space-3xl)' }}>
        ...
        <button onClick={() => nav('/products')} className="btn">
        ...
        <div className="detail-info">
          <div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-md)', marginBottom: 'var(--space-lg)' }}>
              ...
              <span style={{
                color: product.stock > 5 ? 'var(--success)' : 'var(--warning)',
                fontWeight: 600,
                fontSize: '0.9rem'
              }}>
          ...
          <button
            ...
            style={{
              ...
              fontSize: '1.1rem',
              background: addedToCart ? 'var(--success)' : undefined
            }}
          >
          ...
          {product.stock === 0 && (
            <p style={{ color: 'var(--error)', marginTop: 'var(--space-md)', textAlign: 'center' }}>
